...
(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.jslucas = {}));
	...
	*/
	...
	/**
	 * Mask Position Module
	 * Simple centered circle that grows on hover and scroll
	 */

	function initMaskPosition() {
	  const button = document.querySelector('.btn-scroll-on');
	  const cutoutPath = document.querySelector('#cutout-circle');
	  if (!button || !cutoutPath) {
	    console.log('Mask position: waiting for elements...');
	    return;
	  }
	  console.log('🎯 Mask position module initialized');
	  let isHovering = false;

	  // HOVER INTERACTION - grows to button size
	  button.addEventListener('mouseenter', () => {
	    isHovering = true;
	    console.log('🔍 Hover: Growing to button size');
	    cutoutPath.style.transition = 'transform 0.3s ease-out';
	    cutoutPath.style.transform = 'scale(200)'; // Scale to roughly button size
	  });

	  button.addEventListener('mouseleave', () => {
	    isHovering = false;
	    console.log('↩️ Hover end: Returning to CSS scroll control');
	    cutoutPath.style.transform = '';
	  });

	  // SCROLL DETECTION - prioritize scroll animation
	  let lastScrollY = window.scrollY;
	  window.addEventListener('scroll', () => {
	    const currentScrollY = window.scrollY;
	    if (currentScrollY !== lastScrollY && isHovering) {
	      console.log('📜 Scroll detected: prioritizing scroll animation');
	      isHovering = false;
	      cutoutPath.style.transform = '';
	    }
	...
	// NOTE: SplitType above is no longer used by anything in this file — it's
	// dead code, left in place rather than surgically removed from a large
	// bundle. The site now loads the real split-type package from a CDN
	// (layout.astro) instead, used by /js/modules/scroll-controls.js.
	...
	function initCustomCursor() {
	  ...
	  hoverTargets.forEach(el => {
	    el.addEventListener('mouseenter', () => cursor.classList.add('cursor--hovering'));
	    el.addEventListener('mouseleave', () => cursor.classList.remove('cursor--hovering'));
	  });
	...
	/**
	 * Custom JavaScript - Main Entry Point
	 * Imports and initializes all modules
	 *
	 * GSAP-dependent modules that used to be initialized here
	 * (initTextSplitAnimations, initValuesAnimation, initAnimations,
	 * initParallaxShapes, initMouseParallax, initNavAnimation) have moved to
	 * /js/modules/scroll-controls.js, bootstrapped in layout.astro — that's
	 * now the single home for this site's GSAP/ScrollTrigger scroll effects.
	 * Only the non-GSAP modules remain here.
	 */
	// Note: laptop-3d is loaded separately via threejs-setup.php

	// GSAP and ScrollTrigger are loaded globally via WordPress
	// Check they're available before proceeding
	if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
	  gsap.registerPlugin(ScrollTrigger);
	}
	console.log('🚀 Initializing custom JavaScript modules...');
